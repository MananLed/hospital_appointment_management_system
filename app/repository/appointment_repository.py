...
class AppointmentRepository:
    ...
    def get_occupied_timeslots(self, id: UUID, appointment_date: date) -> List[str]:
        ...
        for item in items:
            occupied_timeslots_details = {k: self.deserializer.deserialize(v) for k, v in item.items()}
    ...
    def get_patient_appointments(self, patient_id: str, appointment_date: date) -> List[Appointment]:
    ...
    def book_appointment(self, appointment: Appointment) -> None:
        ...
        patient_sk = (
            f"DATE#{appointment.appointment_date.isoformat()}#DEPARTMENT#{appointment.department}#DOCTOR#UUID#{appointment.doctor_id}#TIME#{appointment.timeslot}"
        )
        ...
        try:
            self.dynamodb.execute_transaction(
                TransactStatements=[
                    {
                        "Statement": statement,
                        "Parameters": [{"S": doctor_pk}, {"S": doctor_sk}] + common_parameters
                    },
                    {
                        "Statement": statement,
                        "Parameters": [{"S": patient_pk}, {"S": patient_sk}] + common_parameters
                    }
                ]
            )
    ...
    def get_all_appointments_of_doctor(self, id: UUID, appointment_date: date, timeslot: str | None = None) -> List[Appointment]:
    ...
    def cancel_appointment(self, appointment: Appointment) -> None:
        ...
        doctor_pk = f"DOCTOR#UUID#{appointment.doctor_id}#DATE#{appointment.appointment_date}"
        doctor_sk = f"TIME#{appointment.timeslot}"

        patient_pk = f"PATIENT#UUID#{appointment.patient_id}"
        patient_sk = (
            f"DATE#{appointment.appointment_date}#DEPARTMENT#{appointment.department}#DOCTOR#UUID#{appointment.doctor_id}#TIME#{appointment.timeslot}"
        )
        ...
        try:
            self.dynamodb.execute_transaction(
                TransactStatements=[
                    {
                        "Statement": statement,
                        "Parameters": [{"S": AppointmentStatus.COMPLETED.value}, {"S": doctor_pk}, {"S": doctor_sk}]
                    },
                    {
                        "Statement": statement,
                        "Parameters": [{"S": AppointmentStatus.COMPLETED.value}, {"S": patient_pk}, {"S": patient_sk}] 
                    }
                ]
            )
        except:
