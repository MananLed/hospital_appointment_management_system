...
class AppointmentService:
    ...
    def get_available_timeslots(
        self, id: UUID, appointment_date: date
    ) -> List[TimeSlot]:
    ...
    def book_appointment(self, appointment: Appointment):
        ...
        patient_appointments: List[Appointment] = (
            self.appointment_repository.get_patient_appointments(
    ...
    def get_all_appointments_of_doctor(
        ...
        return self.appointment_repository.get_all_appointments_of_doctor(
            id, appointment_date
        )

    def get_all_appointments_of_patient(
        self, id: str, appointment_date: date
    ) -> List[Appointment]:

        return self.appointment_repository.get_patient_appointments(
            id, appointment_date
        )
    ...
    def cancel_appointment(
        ...
        appointments: List[Appointment] = (
            self.appointment_repository.get_all_appointments_of_doctor(
                doctor_id, appointment_date, timeslot
            )
        )
        ...
        if patient_id is not None:
            message: str = (
                f"You have successfully cancelled your appointment with Dr. {appointment_details.doctor_name} at {appointment_details.timeslot}."
            )
        else:
            message: str = (
                f"Due to some unavoidable circumstances your appointment with Dr. {appointment_details.doctor_name} at {appointment_details.timeslot} has been cancelled. We apologize for the inconvinience."
            )
    ...
    def mark_appointment_complete(
        self, doctor_id: UUID, appointment_date: date, timeslot: str
    ):
        today = date.today()
