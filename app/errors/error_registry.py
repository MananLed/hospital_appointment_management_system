...
db_exception: ErrorDefinition = ErrorDefinition(
    http_status=status.HTTP_500_INTERNAL_SERVER_ERROR, message=DB_ERROR
)

sys_exception: ErrorDefinition = ErrorDefinition(
    http_status=status.HTTP_500_INTERNAL_SERVER_ERROR, message=SERVER_ERROR
)

unauthorized_exception: ErrorDefinition = ErrorDefinition(
    http_status=status.HTTP_401_UNAUTHORIZED, message=UNAUTHORIZED_ACCESS
)


ERROR_REGISTRY: Dict[str, ErrorDefinition] = {
    HOSPITAL_001: ErrorDefinition(
        http_status=status.HTTP_400_BAD_REQUEST, message="Missing department value"
    ),
    ...
    AUTH_002: ErrorDefinition(
        http_status=status.HTTP_401_UNAUTHORIZED, message="Invalid or expired token"
    ),
    ...
    USER_001: ErrorDefinition(
        http_status=status.HTTP_401_UNAUTHORIZED, message="Invalid Credentials"
    ),
    ...
    APPOINTMENT_005: ErrorDefinition(
        http_status=status.HTTP_409_CONFLICT, message="Timeslot is not valid"
    ),
    ...
    APPOINTMENT_011: ErrorDefinition(
        http_status=status.HTTP_409_CONFLICT, message="Slot is already booked"
    ),
