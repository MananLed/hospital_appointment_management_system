...
def login(login_details: LoginInput):
    ...
    access_data = user_service_instance.get_user_by_email_and_password(
        login_details.email, login_details.password
    )

    return Response.success_response(
        access_data, "Login Successful", status.HTTP_201_CREATED
    )
...
def signup(signup_details: SignUpInput):
    ...
    return Response.success_response(
        None, "Sign Up Successful", status.HTTP_201_CREATED
    )
