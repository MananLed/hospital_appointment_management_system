...
hospital_router = APIRouter(prefix="/admin", dependencies=[Depends(verify_jwt), Depends(require_roles(UserRole.ROLEADMIN))])
...
def add_doctor(doctor_details: SignUpInput):
    ...
    return Response.success_response(None, "Doctor added successfully", status.HTTP_201_CREATED)
